...
import (
	...
	"github.com/lemonsuo/ipScanner/task"
	"github.com/lemonsuo/ipScanner/utils"
)
...
`
	...
	if printVersion {
		...
		checkUpdate()
		if versionNew != "" {
			fmt.Printf("*** Found new version [%s]! Please go to [https://github.com/lemonsuo/ipScanner] to update! ***", versionNew)
		} else {
...
func main() {
	...
	fmt.Printf("# lemonsuo/ipScanner %s \n\n", version)
	...
	if versionNew != "" {
		fmt.Printf("\n*** Found New Version [%s]! Please go to [https://github.com/lemonsuo/ipScanner] to update! ***\n", versionNew)
	}
	endPrint()
}
...
func checkUpdate() {
	...
	res, err := client.Get("https://api.github.com/repos/lemonsuo/ipScanner/releases/latest")
	if err != nil {
		return
	}
