...
[![Go Version](https://img.shields.io/github/go-mod/go-version/lemonsuo/ipScanner.svg?style=flat-square&label=Go&color=00ADD8&logo=go)](https://github.com/lemonsuo/ipScanner/)[![Release Version](https://img.shields.io/github/v/release/lemonsuo/ipScanner.svg?style=flat-square&label=Release&color=00ADD8&logo=github)](https://github.com/lemonsuo/ipScanner/releases/latest)[![GitHub license](https://img.shields.io/github/license/lemonsuo/ipScanner.svg?style=flat-square&label=License&color=00ADD8&logo=github)](https://github.com/lemonsuo/ipScanner/)[![GitHub Star](https://img.shields.io/github/stars/lemonsuo/ipScanner.svg?style=flat-square&label=Star&color=00ADD8&logo=github)](https://github.com/lemonsuo/ipScanner/)[![GitHub Fork](https://img.shields.io/github/forks/lemonsuo/ipScanner.svg?style=flat-square&label=Fork&color=00ADD8&logo=github)](https://github.com/lemonsuo/ipScanner/)
...
1.  Download the compiled executable file [Github Releases](https://github.com/lemonsuo/ipScanner/releases) and unzip it.
2.  Double click to run`ipScanner.exe`File (Windows system), waiting for the speed test to complete...
...
The following commands are only examples. For the version number and file name, please go to [**Releases**](https://github.com/lemonsuo/ipScanner/releases) Check.
...
# Download the ipScanner zip file (replace [version number] and [file name] in the URL according to your needs)
wget -N https://github.com/lemonsuo/ipScanner/releases/download/v2.2.5/ipScanner_linux_amd64.zip
# If you are downloading in a Chinese network environment, please use one of the following mirror accelerations:
# wget -N https://download.scholar.rr.nu/lemonsuo/ipScanner/releases/download/v2.2.5/ipScanner_linux_amd64.zip
# wget -N https://ghproxy.cc/https://github.com/lemonsuo/ipScanner/releases/download/v2.2.5/ipScanner_linux_amd64.zip
# wget -N https://ghproxy.net/https://github.com/lemonsuo/ipScanner/releases/download/v2.2.5/ipScanner_linux_amd64.zip
# wget -N https://gh-proxy.com/https://github.com/lemonsuo/ipScanner/releases/download/v2.2.5/ipScanner_linux_amd64.zip
# wget -N https://mirror.ghproxy.com/https://github.com/lemonsuo/ipScanner/releases/download/v2.2.5/ipScanner_linux_amd64.zip
# If the download fails, try removing the -N parameter (if for updating, remember to delete the old zip file in advance rm ipScanner_linux_amd64.zip)

# Unzip (no need to delete old files, they will be overwritten directly, replace the file name according to your needs)
tar -zxf ipScanner_linux_amd64.zip
...
> If flat**Very low latency**(such as 0.xx), then ipScanner**The agent left during the speed test**, please close the proxy software before testing the speed.  
...
* * *
> Notice! This software only works on websites,**Preferred IP for Cloudflare WARP is not supported**, see for details:[#392](https://github.com/lemonsuo/ipScanner/discussions/392)
...
# If the average latency is very low (e.g., 0.xx), it indicates that ipScanner is using a proxy during the speed test. Please disable any proxy software before conducting the test.
...
https://github.com/lemonsuo/ipScanner
...
> People who are new to ipScanner may be confused**There are obviously 30 IPs available for delay speed testing, so why are there only 3 left in the end?**  
> What does the queue in the download speed test mean? Do I still have to queue up when I download the speed test?

ipScanner will first delay the speed test, during which the number of available IPs will be displayed in real time on the right side of the progress bar (`Available: 30`), but note that the available quantity refers to**Number of IPs that passed the test without timeout**, has nothing to do with the upper and lower limits of delay and packet loss conditions. When the delayed speed test is completed, because it is also specified**Delay upper and lower limits, packet loss**conditions, so after filtering according to the conditions, only`10`Already (that is, waiting for the download speed test)`queue：10`）。
...
> **hint**：Windows**PowerShell**Just put in the following command`ipScanner.exe`Change to`.\ipScanner.exe`That’s it.  
> **hint**: Linux system only needs to change the following command`ipScanner.exe`Change to`./ipScanner`That’s it.

* * *

#### #CMD runs ipScanner with parameters
...
Many people open CMD to**absolute path**Running ipScanner will report an error. This is because the default`-f ip.txt`The parameter is a relative path, and you need to specify the absolute path of ip.txt. However, this is too troublesome, so it is recommended to enter the ipScanner program directory to**relative path**Run as:

**method one**：

1.  Open the directory where the ipScanner program is located
...
4.  Enter a command with parameters, such as:`ipScanner.exe -tll 50 -tl 200`Ready to run
...
3.  Enter a command with parameters, such as:`ipScanner.exe -tll 50 -tl 200`Ready to run

> Of course, you can also open a CMD window at will and enter such as`cd /d "D:\Program Files\ipScanner"`to enter the program directory

> **hint**: If using**PowerShell**Just put in the command`ipScanner.exe`Change to`.\ipScanner.exe`That’s it.

</details>

* * *

#### #Windows shortcut to run ipScanner with parameters
...
Right click`ipScanner.exe`document -**\[Create Shortcut]**, then right-click the shortcut -**\[Attributes]**, modify its**Target**：
...
D:\ABC\ipScanner\ipScanner.exe -n 500 -t 4 -dn 20 -dt 5 -o " "
...
"D:\Program Files\ipScanner\ipScanner.exe" -n 500 -t 4 -dn 20 -dt 5 -o " "
...
ipScanner.exe -f ip.txt

# You can specify the built-in IPv6 data file to test these IPv6 addresses
# Additionally, starting from version 2.1.0, ipScanner supports mixed IPv4+IPv6 speed testing and removes the -ipv6 parameter, so one file can contain both IPv4+IPv6 addresses.
ipScanner.exe -f ipv6.txt

# You can also directly specify the IP addresses to be tested through parameters
ipScanner.exe -ip 1.1.1.1,2606:4700::/32
```

> When measuring IPv6 speed, you may notice that the number of speed measurements is different each time. Learn the reason:[#120](https://github.com/lemonsuo/ipScanner/issues/120)  
...
ipScanner.exe -cfcolo HKG,KHH,NRT,LAX,SEA,SJC,FRA,MAD
...
> The two CDN airport three-character codes are common, so each region name is visible:<https://www.ipScanneratus.com/>
...
ipScanner.exe -f 2.txt -o "" -p 10 -dd
...
ipScanner.exe -f 3.txt -o result.txt -dd
...
# Linux (in the abc folder within the ipScanner program directory)
./ipScanner -f abc/3.txt -o abc/result.txt -dd

# Windows (note the backslash)
ipScanner.exe -f abc\3.txt -o abc\result.txt -dd
...
./ipScanner -f /abc/4.txt -o /abc/result.csv -dd

# Windows (note the backslash)
ipScanner.exe -f C:\abc\4.txt -o C:\abc\result.csv -dd


# If you want to run ipScanner with an absolute path, then the filenames in the -f / -o parameters must also be absolute paths, otherwise an error will occur indicating that the file cannot be found!
# Linux (in the /abc/ directory)
/abc/ipScanner -f /abc/4.txt -o /abc/result.csv -dd

# Windows (note the backslash)
C:\abc\ipScanner.exe -f C:\abc\4.txt -o C:\abc\result.csv -dd
```
...
ipScanner.exe -tp 80 -url http://cdn.cloudflare.steamstatic.com/steam/apps/5952/movie_max.webm
...
ipScanner.exe -url https://cf.xiu2.xyz/url
...
ipScanner.exe -tl 200 -sl 5.6 -dn 10
```
...
```bash
# First navigate to the directory where ipScanner is located, then run:
# For Windows systems (run in CMD)
ipScanner.exe -ip 1.1.1.1,2.2.2.2/24,2606:4700::/32

# For Linux systems
./ipScanner -ip 1.1.1.1,2.2.2.2/24,2606:4700::/32
```
...
Then add startup parameters when running ipScanner`-f 1.txt`to specify the IP segment data file.

```bash
# First navigate to the directory where ipScanner is located, then run:
# For Windows systems (run in CMD)
ipScanner.exe -f 1.txt
...
```bash
# Run this command in the ipScanner directory via the command line (e.g., CMD, Bat script) to compile a binary executable program that can run in an environment with the same system, bitness, and architecture as the current device (Go will automatically detect your system bitness and architecture), and the version number will be v2.2.5.
...
SET GOOS=linux
SET GOARCH=amd64
go build -o Releases\ipScanner_linux_amd64\ipScanner -ldflags "-s -w -X main.version=%version%"
```
...
GOOS=windows
GOARCH=386
go build -o Releases/ipScanner_windows_386/ipScanner.exe -ldflags "-s -w -X main.version=${version}"
```
